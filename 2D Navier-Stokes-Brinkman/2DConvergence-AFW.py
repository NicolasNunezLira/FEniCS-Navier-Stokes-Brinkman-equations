...
fileO = XDMFFile("Outputs2D/convergence-AFW-2D.xdmf")
...
for nk in range(nkmax):
    ...
    sigh_v = project(sigmah, Th)
    
    th_v = project(th, Th)
    
    gammah_v = project(th, Th)
    
    # saving to file

    th_v.rename("t","t"); fileO.write(th_v,nk*1.0)
    uh.rename("u","u"); fileO.write(uh,nk*1.0)
    sigh_v.rename("sig","sig"); fileO.write(sigh_v,nk*1.0)
    gammah_v.rename("gam","gam"); fileO.write(gammah_v,nk*1.0)
